...
function generarGrupos() {
    const nombreIntegrantesInput = document.getElementById('nombreIntegrantes');
    const numeroIntegrantesInput = document.getElementById('numeroIntegrantes');
...
function creearGrupos(nombreIntegrantes, numeroIntegrantes) {
    const cantidadMinimaIntegrantes = 2;
    const grupos = Array.from({ length: numeroIntegrantes }, () => []);
...
function mostrarGrupos(grupos, container) {
    if (!container)
        return;
    container.innerHTML = '';
